...
exports.schemaKeys = joi.object({
  ...
  exterior: joi.number().integer().allow(0),
  cost: joi.string().allow(null).allow(''),
  durationDays: joi.number().integer().allow(0),
...
exports.findFilterKeys = joi.object({
  ...
  ...Object.fromEntries(
    keys.map(key => [key, joi.object({
      ...
      exterior: joi.alternatives().try(joi.array().items(),joi.number().integer(),joi.object()),
      cost: joi.alternatives().try(joi.array().items(),joi.string(),joi.object()),
      durationDays: joi.alternatives().try(joi.array().items(),joi.number().integer(),joi.object()),
