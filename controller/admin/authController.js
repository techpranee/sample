...
const register = async (req,res) =>{
    try {
        ...
        let checkUniqueFields = await common.checkUniqueFieldsInDatabase(User,[ 'email', 'contact.phone' ],data,'REGISTER');
    ...
    */
    const login = async(req,res)=>{
        try {
            ...
            return res.success({ data: result.data,message:'Login Successful' });
        } catch (error) {
            return res.internalServerError({data:error.message}); 
        }
    }
...
const logout = async (req, res) => {
    try {
        let userToken = await dbService.findOne(userTokens, { token: (req.headers.authorization).replace('Bearer ', '') ,userId:req.user.id});
        let updatedDocument = {
            isTokenExpired: true
        }
...
module.exports = {
    register,sendOtpForLogin,loginWithOTP,login,forgotPassword,validateResetPasswordOtp,resetPassword,logout,addPlayerId,removePlayerId
}