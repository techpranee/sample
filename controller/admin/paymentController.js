...
/**
 * @description : partially update document of Payment with data by id;
 * @param {obj} req : request including id in request params and data in request body.
 * @param {obj} res : response of updated Payment.
 * @return {obj} : updated Payment. {status, message, data}
 */
const partialUpdatePayment = async (req,res) => {
  try {
    if (!req.params.id){
...
const softDeletePayment = async (req,res) => {
  try {
    ...
    let updatedPayment = await deleteDependentService.softDeletePayment(query, updateBody);
    if (!updatedPayment){
      return res.recordNotFound();
    }
    return res.success({ data:updatedPayment });
...
const deletePayment = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedPayment = await deleteDependentService.countPayment(query);
    } else {
      deletedPayment = await deleteDependentService.deletePayment(query);
    }
    if (!deletedPayment){
      return res.recordNotFound();
    }
