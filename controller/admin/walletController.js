...
const deleteWallet = async (req,res) => {
  try { 
    ...
    const query = { _id:req.params.id };
    const deletedWallet = await dbService.deleteOne(Wallet, query);
    if (!deletedWallet){
      return res.recordNotFound();
    }
...
const deleteManyWallet = async (req, res) => {
  try {
    ...
    const deletedWallet = await dbService.deleteMany(Wallet,query);
    if (!deletedWallet){
      return res.recordNotFound();
    }
    return res.success({ data :{ count :deletedWallet } });
  } catch (error){
    return res.internalServerError({ message:error.message }); 
  }
};
