...
const auth = (platform) => async (req, res, next) => {
  ...
  if (platform == PLATFORM.USERAPP){
    ...
    return new Promise((resolve, reject) => {
      passport.authenticate('cleanerapp-rule', { session: false }, verifyCallback(req, resolve, reject, platform))(
        req,
        res,
        next
      );
    ...
    return new Promise((resolve, reject) => {
      passport.authenticate('dashboard-rule', { session: false }, verifyCallback(req, resolve, reject, platform))(
        req,
        res,
        next
      );
